	...
	(function(global, factory) {
		...
		*/
		const getInput = (popup, inputType) => {
			...
			switch (inputType) {
				case "select":
				case "textarea":
				case "file": return popup.querySelector(".".concat(swalClasses.popup, " > .").concat(swalClasses[inputType]));
				case "checkbox": return popup.querySelector(".".concat(swalClasses.popup, " > .").concat(swalClasses.checkbox, " input"));
				case "radio": return popup.querySelector(".".concat(swalClasses.popup, " > .").concat(swalClasses.radio, " input:checked")) || popup.querySelector(".".concat(swalClasses.popup, " > .").concat(swalClasses.radio, " input:first-child"));
				case "range": return popup.querySelector(".".concat(swalClasses.popup, " > .").concat(swalClasses.range, " input"));
				default: return popup.querySelector(".".concat(swalClasses.popup, " > .").concat(swalClasses.input));
			}
		};
		/**
		...
		*/
		...
		const animationEndEvent = (() => {
			...
			for (const i in transEndEventNames) if (Object.prototype.hasOwnProperty.call(transEndEventNames, i) && typeof testEl.style[i] !== "undefined") return transEndEventNames[i];
			return false;
		})();
		...
		*/
		...
		renderInputType.checkbox = (checkboxContainer, params) => {
			...
			setInnerHtml(checkboxContainer.querySelector("span"), params.inputPlaceholder);
		...
		const setContent = (icon, params) => {
			...
			else if (params.icon === "success") setInnerHtml(icon, successIconHtml);
			else if (params.icon === "error") setInnerHtml(icon, errorIconHtml);
			else setInnerHtml(icon, iconContent({
				question: "?",
				warning: "!",
				info: "i"
			}[params.icon]));
		...
		const getTemplateParams = (params) => {
			...
			return Object.assign(getSwalParams(templateContent), getSwalButtons(templateContent), getSwalImage(templateContent), getSwalIcon(templateContent), getSwalInput(templateContent), getSwalStringParams(templateContent, swalStringParams));
		};
		/**
		...
		*/
		const showWarningsForAttributes = (el, allowedAttributes) => {
			toArray(el.attributes).forEach((attribute) => {
				if (allowedAttributes.indexOf(attribute.name) === -1) warn(["Unrecognized attribute \"".concat(attribute.name, "\" on <").concat(el.tagName.toLowerCase(), ">."), "".concat(allowedAttributes.length ? "Allowed attributes are: ".concat(allowedAttributes.join(", ")) : "To set the value, use HTML within the element.")]);
			});
		};
		...
		*/
		...
		const handlePopupClick = (instance, domCache, dismissWith) => {
			if (privateProps.innerParams.get(instance).toast) handleToastClick(instance, domCache, dismissWith);
			else {
		...
		*/
		...
		const argsToParams = (args) => {
			...
			if (typeof args[0] === "object" && !isElement(args[0])) Object.assign(params, args[0]);
				...
				else if (arg !== void 0) error("Unexpected type of ".concat(name, "! Expected \"string\" or \"Element\", got ").concat(typeof arg));
			});
			return params;
		};
		function fire() {
