...
import { Bt as onUnmounted, Gn as reactive, It as onMounted, W as computed, _n as watch, er as toRefs, qn as ref } from "./runtime-dom.esm-bundler-CeqIid9a.js";
import "./vue.js";
...
function getInputFiles(evt) {
	return (isInput(evt.target) ? evt.target.files ? fromList(evt.target.files) : [] : []).map(function(file) {
...
//#endregion
//#region node_modules/vue3-dropzone/dist/index.esm.js
...
var acceptsFn = import_es.default.default || import_es.default;
...
function composeEventHandlers() {
	...
	return function(event) {
		...
		return fns.some(function(fn) {
			if (!isPropagationStopped(event) && fn) fn.apply(void 0, __spreadArray([event], args, false));
...
function useDropzone(options) {
	...
	var onWindowFocus = function() {
		...
		if (state.isFileDialogActive) setTimeout(function() {
			if (inputRef.value) {
	...
	function onClickCb() {
		if (optionsRef.value.noClick) return;
	...
	var onDocumentDrop = function(event) {
		...
		if ((rootRef.value.$el || rootRef.value).contains(event.target)) return;
		event.preventDefault();
		dragTargetsRef.value = [];
	};
	onMounted(function() {
		window.addEventListener("focus", onWindowFocus, false);
		if (optionsRef.value.preventDropOnDocument) {
	...
	function stopPropagation(event) {
		if (optionsRef.value.noDragEventsBubbling) event.stopPropagation();
	}
			...
			return __generator(this, function(_b) {
				switch (_b.label) {
					case 0:
						...
						if (!getFilesFromEvent) return [2];
						return [4, getFilesFromEvent(event)];
					case 1:
						draggedFilesRes = _b.sent();
						if (!draggedFilesRes) draggedFilesRes = [];
						if (isPropagationStopped(event) && !noDragEventsBubbling) return [2];
	...
	function onDragOverCb(event) {
		...
		event.preventDefault();
		stopPropagation(event);
		if (event.dataTransfer) try {
			event.dataTransfer.dropEffect = "copy";
		} catch (_a) {}
		if (isEvtWithFiles(event) && onDragOver) onDragOver(event);
		return false;
	}
	function onDragLeaveCb(event) {
		...
		var targets = dragTargetsRef.value.filter(function(target) {
			...
			return (rootRef.value.$el || rootRef.value).contains(target);
		});
	...
	var getRootProps = function(_a) {
		if (_a === void 0) _a = {};
		var onFocus = _a.onFocus, onBlur = _a.onBlur, onClick = _a.onClick, onDragEnter = _a.onDragEnter, onDragenter = _a.onDragenter, onDragOver = _a.onDragOver, onDragover = _a.onDragover, onDragLeave = _a.onDragLeave, onDragleave = _a.onDragleave, onDrop = _a.onDrop, rest = __rest(_a, [
			"onFocus",
			"onBlur",
			"onClick",
			"onDragEnter",
		...
		return __assign(__assign({
			...
			ref: rootRef
		}, !optionsRef.value.disabled && !optionsRef.value.noKeyboard ? { tabIndex: 0 } : {}), rest);
	};
	...
	return __assign(__assign({}, toRefs(state)), {
		isDragAccept,
		isDragReject,
		isFocused: computed(function() {
