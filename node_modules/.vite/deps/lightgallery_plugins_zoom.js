//#region node_modules/lightgallery/plugins/zoom/lg-zoom.es5.js
/*!
...
var Zoom = function() {
	...
	*/
	Zoom.prototype.enableZoom = function(event) {
		...
		if (this.$LG("body").first().hasClass("lg-from-hash") && event.detail.delay) _speed = 0;
		else this.$LG("body").first().removeClass("lg-from-hash");
		this.zoomableTimeout = setTimeout(function() {
			if (!_this.isImageSlide(_this.core.index)) return;
	...
	Zoom.prototype.getDragAllowedAxises = function(scale, scaleDiff) {
		...
		return {
			allowX: width > this.containerRect.width,
			allowY
		};
	};
	...
	*/
	Zoom.prototype.zoomImage = function(scale, scaleDiff, reposition, resetToMax) {
		...
		var originalX;
		var originalY;
		if (scale === 1) this.positionChanged = false;
		...
		if (reposition) {
			if (allowX) {
				if (this.isBeyondPossibleLeft(x, possibleSwipeCords.minX)) x = possibleSwipeCords.minX;
				else if (this.isBeyondPossibleRight(x, possibleSwipeCords.maxX)) x = possibleSwipeCords.maxX;
			} else if (scale > 1) {
				if (x < possibleSwipeCords.minX) x = possibleSwipeCords.minX;
				else if (x > possibleSwipeCords.maxX) x = possibleSwipeCords.maxX;
			}
			if (allowY) {
				if (this.isBeyondPossibleTop(y, possibleSwipeCords.minY)) y = possibleSwipeCords.minY;
				else if (this.isBeyondPossibleBottom(y, possibleSwipeCords.maxY)) y = possibleSwipeCords.maxY;
			} else if (scale > 1) {
				if (y < possibleSwipeCords.minY) y = possibleSwipeCords.minY;
		...
		this.left = x;
		this.top = y;
		if (resetToMax) this.setZoomImageSize();
	};
	Zoom.prototype.resetImageTranslate = function(index) {
		if (!this.isImageSlide(index)) return;
	...
	Zoom.prototype.setZoomImageSize = function() {
		...
		setTimeout(function() {
			...
			if (_this.scale >= actualSizeScale) {
				...
				if (dragAllowedAxises.allowX && dragAllowedAxises.allowY) $image.addClass("reset-transition");
				else if (dragAllowedAxises.allowX && !dragAllowedAxises.allowY) $image.addClass("reset-transition-x");
				else if (!dragAllowedAxises.allowX && dragAllowedAxises.allowY) $image.addClass("reset-transition-y");
			}
	...
	*/
	Zoom.prototype.setActualSize = function(index, event) {
		...
		setTimeout(function() {
			if (!currentItem.src || _this.core.outer.hasClass("lg-first-slide-loading")) return;
			var scale = _this.getCurrentImageActualSizeScale();
			var prevScale = _this.scale;
			if (_this.core.outer.hasClass("lg-zoomed")) _this.scale = 1;
			else _this.scale = _this.getScale(scale);
	...
	Zoom.prototype.getCurrentImageActualSizeScale = function() {
		var width = this.core.getSlideItem(this.core.index).find(".lg-image").first().get().offsetWidth;
	...
	Zoom.prototype.init = function() {
		...
		this.core.outer.on("touchstart.lg", function(event) {
			...
			if (event.touches.length === 1 && $target.hasClass("lg-image")) if (!tapped) tapped = setTimeout(function() {
				tapped = null;
			}, 300);
			else {
				clearTimeout(tapped);
		...
		this.core.LGel.on(lGEvents.containerResize + ".zoom " + lGEvents.rotateRight + ".zoom " + lGEvents.rotateLeft + ".zoom " + lGEvents.flipHorizontal + ".zoom " + lGEvents.flipVertical + ".zoom", function() {
			if (!_this.core.lgOpened || !_this.isImageSlide(_this.core.index) || _this.core.touchAction) return;
		...
		this.core.LGel.on(lGEvents.afterSlide + ".zoom", function(event) {
			...
			if (_this.isImageSlide(_this.core.index)) _this.setZoomEssentials();
		});
		this.zoomDrag();
		this.pinchZoom();
		this.zoomSwipe();
	...
	Zoom.prototype.zoomIn = function() {
		if (!this.isImageSlide(this.core.index)) return;
		var scale = this.scale + this.settings.scale;
		if (!this.settings.infiniteZoom) scale = this.getScale(scale);
	...
	Zoom.prototype.pinchZoom = function() {
		...
		this.core.outer.on("touchstart.lg", function(e) {
			...
			if (e.touches.length === 2) {
				e.preventDefault();
				if (_this.core.outer.hasClass("lg-first-slide-loading")) return;
		...
		this.core.$inner.on("touchmove.lg", function(e) {
			if (e.touches.length === 2 && _this.core.touchAction === "pinch" && (_this.$LG(e.target).hasClass("lg-item") || $item.get().contains(e.target))) {
				...
				if (pinchStarted) {
					...
					var _scale = Math.max(1, initScale + -distance * .02);
		...
		this.core.$inner.on("touchend.lg", function(e) {
			if (_this.core.touchAction === "pinch" && (_this.$LG(e.target).hasClass("lg-item") || $item.get().contains(e.target))) {
				pinchStarted = false;
				startDist = 0;
				if (_this.scale <= 1) _this.resetZoom();
				else {
					var actualSizeScale = _this.getCurrentImageActualSizeScale();
					if (_this.scale >= actualSizeScale) {
						var scaleDiff = actualSizeScale - _this.scale;
						if (scaleDiff === 0) scaleDiff = .01;
	...
	Zoom.prototype.touchendZoom = function(startCoords, endCoords, allowX, allowY, touchDuration) {
		...
		if (Math.abs(distanceXnew) > 15 || Math.abs(distanceYnew) > 15) {
			if (allowY) {
				if (this.isBeyondPossibleTop(distance.y, possibleSwipeCords.minY)) distance.y = possibleSwipeCords.minY;
				else if (this.isBeyondPossibleBottom(distance.y, possibleSwipeCords.maxY)) distance.y = possibleSwipeCords.maxY;
			}
			if (allowX) {
				if (this.isBeyondPossibleLeft(distance.x, possibleSwipeCords.minX)) distance.x = possibleSwipeCords.minX;
	...
	Zoom.prototype.getPossibleSwipeDragCords = function(scale) {
		...
		return {
			minY: (imageHeight - this.containerRect.height) / 2,
			maxY: (this.containerRect.height - imageHeight) / 2 + bottom,
			minX: (imageWidth - this.containerRect.width) / 2,
			maxX: (this.containerRect.width - imageWidth) / 2
		};
	};
	...
	Zoom.prototype.zoomSwipe = function() {
		...
		this.core.$inner.on("touchstart.lg", function(e) {
			...
			if ((_this.$LG(e.target).hasClass("lg-item") || $item.get().contains(e.target)) && e.touches.length === 1 && _this.core.outer.hasClass("lg-zoomed")) {
				...
				if (allowX || allowY) startCoords = _this.getSwipeCords(e);
