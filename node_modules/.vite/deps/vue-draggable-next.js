import { It as onMounted, W as computed, Xt as resolveComponent, dt as getCurrentInstance, kt as nextTick, mt as h, qn as ref, rt as defineComponent } from "./runtime-dom.esm-bundler-CeqIid9a.js";
import "./vue.js";
//#region node_modules/vue-draggable-next/dist/vue-draggable-next.esm-bundler.js
/*!
...
function computeIndexes(slots, children, isTransition, footerOffset) {
	...
	return [...children].map((elt, idx) => idx >= footerIndex ? elmFromNodes.length : elmFromNodes.indexOf(elt));
}
	...
	setup(props, { emit, slots, attrs }) {
		...
		function updateOptions(newOptionValue) {
			...
			for (const property in newOptionValue) {
				...
				if (readonlyProperties.indexOf(value) === -1) sortableInstance.value.option(value, newOptionValue[property]);
			}
		}
		...
		function getUnderlyingVm(htmlElt) {
			...
			return {
				index,
				element: realList.value ? realList.value[index] : null
			};
		}
		...
		function updateProperty(evt, propertyName) {
			if (Object.prototype.hasOwnProperty.call(evt, propertyName)) evt[propertyName] += headerOffset.value;
		}
		...
		function getRelatedContextFromMoveEvent({ to, related }) {
			...
			if (to !== related && list && component.getUnderlyingVm) {
				...
				if (destination) return Object.assign(destination, contextLocal);
			}
			return contextLocal;
		}
		...
		const mounted = () => {
			...
			const options = Object.assign({}, attributes, optionsAdded, { onMove: (evt, originalEvent) => {
		...
		function delegateAndEmit(evtName) {
			return (evtData) => {
				if (realList.value !== null) {
					const handler = methods["onDrag" + evtName];
					if (handler) handler(evtData);
				}
		...
		function onDragMove(evt, originalEvent) {
			...
			if (!onMove || !realList.value) return true;
			...
			if (draggedContext) Object.assign(draggedContext, { futureIndex });
			return onMove(Object.assign({}, evt, {
				relatedContext,
				draggedContext
			}), originalEvent);
		}
		...
		if (props.list !== null && props.modelValue !== null) console.error("list props are mutually exclusive! Please set one.");
		return {
			getTag,
			realList,
			visibleIndexes,
...
function validateProps(props) {
	const errors = [];
	if (props.list && props.modelValue) errors.push("Cannot use both \"list\" and \"modelValue\" props simultaneously");
	if (props.animation && (props.animation < 0 || props.animation > 3e3)) errors.push("Animation duration should be between 0 and 3000ms");
	if (props.delay && props.delay < 0) errors.push("Delay must be a positive number");
	return errors;
}
//#endregion
