...
const { saving, currentCategory, categoryLoading } = storeToRefs(categoryStore);
...
onMounted(async () => {
    ...
    if (currentCategory.value) {
        form.nom = currentCategory.value.nom;
    }
});
...
const submit = async () => {
  ...
  const id = Number(route.params.id);
  const result = await categoryStore.updateCategory(id, form);
...
<template>
  <div class="container-fluid">
    ...
    <div class="d-flex align-items-center mb-4 flex-wrap">
      <h3 class="me-auto">Edit Category</h3>
    ...
    <!-- Loading -->
    <div v-if="categoryLoading" class="text-center py-5">
