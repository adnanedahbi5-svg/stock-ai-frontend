...
const { saving, currentFournisseur, fournisseurLoading } = storeToRefs(fournisseurStore);
...
onMounted(async () => {
    ...
    await fournisseurStore.fetchFournisseur(id);
    if (currentFournisseur.value) {
        form.nom = currentFournisseur.value.nom;
        form.contact = currentFournisseur.value.contact;
        form.adresse = currentFournisseur.value.adresse ?? "";
    }
});
...
const submit = async () => {
  ...
  const id = Number(route.params.id);
  const result = await fournisseurStore.updateFournisseur(id, form);

  if (result && result.success) {
    successMsg.value = "Fournisseur updated successfully!";
    setTimeout(() => {
...
<template>
  <div class="container-fluid">
    ...
    <div class="d-flex align-items-center mb-4 flex-wrap">
      <h3 class="me-auto">Edit Fournisseur</h3>
    ...
    <!-- Loading -->
    <div v-if="fournisseurLoading" class="text-center py-5">
      <i class="fas fa-spinner fa-spin fa-2x text-primary"></i>
      <p class="mt-2 text-muted">Loading fournisseur...</p>
    </div>
